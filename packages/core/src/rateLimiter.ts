...
export class RateLimiter {
   ...
   */
  async checkLimit(service: string): Promise<{ allowed: boolean; retryAfter?: number; remaining?: number }> {
   ...
   */
  getStatus(service: string): { remaining: number; resetTime: number; isLimited: boolean } {
    const config = this.configs.get(service);
    const entry = this.limits.get(service);
   ...
   */
  getAllStatuses(): Record<string, ReturnType<typeof this.getStatus>> {
    const statuses: Record<string, ReturnType<typeof this.getStatus>> = {};
